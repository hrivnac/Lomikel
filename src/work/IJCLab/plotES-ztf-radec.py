...
def normalize_locations(value):
    ...
    For ZTF radec documents there may be one or several points, but this
...
def collect_all_points(es_url, index, field, batch_size=1000):
    """
    Scan the ZTF radec index and collect all RA/Dec points.
    ...
    try:
        while True:
            ...
            for hit in hits:
                ...
                if not locations:
                    continue
                # ZTF may have one or more points. Plot them all.
...
def plot_points(points, output=None, invert_ra=False, marker_size = 1.0, alpha = 0.6):
    ...
    plt.xlabel("ra [deg]")
    plt.ylabel("dec [deg]")
    plt.title(f"ZTF object positions, {len(points)} points")
    plt.grid(True)
    if invert_ra:
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot ZTF object RA/Dec positions from Elasticsearch geo_point index."
    )
