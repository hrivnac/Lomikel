...
def normalize_locations(value):
    ...
    For ZTF radec documents there may be one or several points.
    """
    if value is None:
        return []
...
def max_mjd(value):
    """
    ZTF mjd may contain either one MJD/JD value or an array of values.
...
def find_last_dia_ids_by_mjd(es_url, mjd_index, mjd_field, n=100000, batch_size=2000):
    """
    Find the last N ZTF objects according to their maximum MJD/JD.

    This scans mjd and keeps the top N documents by max(mjd).
    ...
    print(f"Scanned ZTF mjd documents: {n_docs}")
...
def mget_radec_points(es_url, radec_index, location_field, mjd_id_pairs, chunk_size=1000):
    """
    Fetch radec locations for selected ZTF object ids.
    ...
    for i in range(0, len(ids), chunk_size):
        ...
        for doc in data.get("docs", []):
            ...
            # ZTF may have one or more points. Plot all.
...
def plot_points(points, output=None, invert_ra=False, marker_size=1.0, alpha=0.6):
    if not points:
        raise RuntimeError("No ZTF ra/dec points to plot.")
    ...
    plt.ylabel("dec [deg]")
    plt.title(
        f"Latest ZTF object positions, {len(points)} points\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot latest ZTF object RA/Dec positions selected by mjd."
    )
