...
logg.info("Processing ${newTags.size()} NewTags")
...
grouped.each {objectId, clsMap ->
  ...
  merged.each {cls, data ->
    ...
    g.E(data.edge.id()).property('instances', data.instances)
...
if (!processedTagIds.isEmpty()) {
  g.V(processedTagIds).property('processed', true)
                      .iterate()
  }
gr.commit()
...
NotifierURL.notifyExecution("processTags-LSST", "Lomikel", Info.release(), timer.info(""));
