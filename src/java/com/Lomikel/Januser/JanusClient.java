...
public class JanusClient {
  ...
  public static void main(String[] args) throws Exception {
    ...
    else if (args[0].trim().equals("populateh")) {
      String failedKey = args[9];
      do {
        failedKey = new JanusClient(args[1]).populateGraphFromHBase(            args[2],
                                                                    new Integer(args[3]),
                                                                                args[4],
                                                                                args[5],
                                                                                args[6],
    ...
    else if (args[0].trim().equals("populates")) {
      String failedKey = args[9];
      do {
        failedKey = new JanusClient(args[1]).populateGraphFromPhoenix(            args[2],
                                                                      new Integer(args[3]),
                                                                                  args[4],
                                                                                  args[5],
                                                                                  args[6],
  ...
  /** Populate JanusGraph from Phoenix table (via proxy).
    * @param proxyHost       The Phoenix proxy hostname.
    * @param proxyPort       The Phoenix proxy port.
    * @param phoenixTable    The Phoenix table to replicate in Graph.
    * @param tableSchema     The Phoenix table schema name.
    * @param label           The label of newly created Vertexes.
    * @param rowkey          The row key name.
    * @param keyPrefixSearch The key prefix to limit replication to.
    * @param keyStart        The key to start search from, may be blank.
  ...
  // TBD: read only rowkey if fullFill = false
  // TBD: handle binary columns
  public String populateGraphFromPhoenix(String  proxyHost,
                                         int     proxyPort,
                                         String  phoenixTable,
                                         String  tableSchema,
                                         String  label,
                                         ...
                                         boolean getOrCreate,
                                         boolean fullFill) throws LomikelException {
    log.info("Populating Graph from " + phoenixTable + "(" + tableSchema + ")@" + proxyHost + ":" + proxyPort);
    log.info("\tvertex labels: " + label);
    log.info("\t" + rowkey + " starting with " + keyPrefixSearch);
    log.info("\tlimit/skip/commitLimit: " + limit + "/" + skip + "/" + commitLimit);
    if (reset) {
